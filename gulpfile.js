...
gulp.task('ejs', () => gulp
  .src('src/*.ejs')
  .pipe(ejs(yaml.load(readFileSync('./src/cv.yaml'))))
  .pipe(
    rename({ extname: '.html' })
  )
  .pipe(gulp.dest('./dist'))
...
gulp.task('serve', () => {
  ...
  // watch SCSS
  gulp
    .watch(['node_modules/bootstrap/scss/bootstrap.scss', 'src/scss/*.scss'])
    .on('change', gulp.series('sass', browserSync.reload))

  // watch EJS
  gulp
    .watch(['src/*.ejs', 'src/cv.yaml'])
    .on('change', gulp.series('ejs', browserSync.reload))
})

gulp.task('clean', () =>
  deleteAsync(['build', 'dist'])
)
